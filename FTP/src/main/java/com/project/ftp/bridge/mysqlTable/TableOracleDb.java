...
public class TableOracleDb implements TableDb {
    private final static Logger logger = LoggerFactory.getLogger(TableOracleDb.class);
//    private final MysqlConnection mysqlConnection;
    private final OracleConnection oracleConnection;
    public TableOracleDb(DataSourceFactory dataSourceFactory, OracleDatabaseConfig oracleDatabaseConfig) {
...
//            finalQueryParam.add(str);
    ...
    private boolean isDeletedEnabled(TableConfiguration tableConfiguration) {
    ...
    public JdbcQueryStatus updateTableEntry(TableConfiguration tableConfiguration, HashMap<String, String> data,
        ...
        String query = "UPDATE " + tableConfiguration.getTableName() + " SET " +
                ...
                " WHERE " + filterQuery;
        try {
            return oracleConnection.updateQueryV2(query, finalQueryParam);
        } catch (Exception e) {
    ...
    public ArrayList<HashMap<String, String>> searchData(TableConfiguration tableConfiguration, HashMap<String, String> filterData) {
        if (tableConfiguration == null || filterData == null) {
            return null;
        }
        ArrayList<String> uniquePattern = tableConfiguration.getUniquePattern();
        if (uniquePattern == null || uniquePattern.isEmpty()) {
            logger.info("searchData: Configuration error. uniquePattern is null or empty: {}, data: {}", uniquePattern, filterData);
            return null;
        }
        ...
        for(String columnName: uniquePattern) {
            columnValue = filterData.get(columnName);
